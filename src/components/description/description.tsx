...
const Description = () => {
  ...
  return (
    <ResponsiveContentContainer>
      ...
      <div className="h-full xl:py-44 lg:py-20 sm:py-10 lg:w-[85%] md:w-full" id="features">
        <div className="flex justify-center items-center">
          <div className="flex flex-col gap-5 justify-center items-center font-helvetica text-black-main lg:w-[80%] md:w-full">
            <Heading tag='h2' className='text-center uppercase font-patrizia'>
              {locale === 'ENG' ? 'UNIQUE FEATURES OF THE HOTEL PROJECT' : 'CARATTERISTICHE UNICHE DEL PROGETTO ALBERGHIERO'}
            </Heading>
            <p className="font-normal text-responsive  text-center">{locale === 'ENG' ? "Nestled in breathtaking natural surroundings, Grand Hotel Pigna is evolving into a luxurious 5-star wellness retreat, embracing a new medical spa concept that fosters loyalty and encourages repeat visits.It is not merely a luxury destination but also a comprehensive thermal resort offering an extensive range of spa treatments and holistic wellness programmes with use of therapeutic thermal water.": 
            ...
            <p className="font-normal text-responsive   text-center">{locale === 'ENG' 
        ...
        <div className="mt-7 w-full flex justify-between items-center sm:flex lg:hidden">
          {descriptionEls.slice(0, 5).map((el, index) => (
            <>
              {!el.isDivider ? <div key={index} className="flex items-center gap-4">
              ...
              </div> : <div className="h-[40px] w-[1px] bg-gray-300" />}
            </>
          ))}
        </div>
        <div className="mt-7 w-full flex justify-between items-center sm:flex lg:hidden">
