...
const tabsResponsive = [
  {
    ...
    descriptionIt: 'I benefici terapeutici dei nostri bagni termali sono noti fin dall’epoca romana. L’acqua termale solfurea tratta problemi dell’apparato muscoloscheletrico, del sistema cardiovascolare, dell’apparato respiratorio, problemi dermatologici ed estetici, obesità; riduce lo stress e elimina le tossine.'
  },
  {
    ...
    descriptionIt: 'Diagnostica completa, un team di medici di livello mondiale e attrezzature mediche all’avanguardia garantiscono cure personalizzate e piani di trattamento su misura.'
  },
  {
    ...
    descriptionIt: 'Strutture dedicate alla riabilitazione, progettate per affrontare infortuni sportivi e disturbi muscoloscheletrici, favorendo il recupero e il rafforzamento.'
  },
  {
    ...
    descriptionIt: 'I nostri programmi di benessere si basano su una fusione di pratiche orientali e occidentali, sulle tecnologie più nuove e sulle ultime ricerche mediche certificate, con un approccio olistico che migliora il benessere mentale, fisico, emotivo e spirituale.'
  },
]
    ...
    <div className="2lg:pt-40 sm:pt-14 w-full bg-loading-color 2lg:pb-44 pb-24" id="wellness-spa">
      <div className="flex justify-center items-center px-[18px]">
        <div className="flex flex-col gap-4 justify-center items-center text-main-white 2lg:w-1/2 sm:w-full">
          ...
          <p className="lg:text-lg sm:text-xs font-helvetica text-center opacity-80">
            {locale === 'ENG' ?
        ...
        <div className='flex relative flex-col gap-4 w-[1750px] 2lg:mt-20 sm:mt-2'>
          ...
          <div className='2lg:flex sm:hidden justify-between items-center gap-4 mt-10'>
            {tabs.map((tab, index) => (
              ...
              >
                <span className='2xl:text-xl uppercase font-helvetica'>{locale === 'ENG' ? tab.title : tab.titleIt}</span>
                <p className='2xl:text-base font-helvetica'>{locale === 'ENG' ? tab.description : tab.descriptionIt}</p>
              </div>
            ))}
          </div>
          ...
          <div className='sm:flex flex-col gap-4 md:hidden'>
            {tabsResponsive.map(tab =>
              <div className='flex flex-col gap-4 font-helvetica w-full justify-start cursor-pointer transition-all duration-300 text-main-white'>
                ...
                />
                <span className='text-main-white text-base w-full uppercase'>{locale === 'ENG' ? tab.title : tab.titleIt}</span>
                <p className='text-main-white text-xs w-full'>{locale === 'ENG' ? tab.description : tab.descriptionIt}</p>
              </div>
            )}
          </div>
